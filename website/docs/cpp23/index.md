...
sidebar_position: 1
title: "C++23 新特性概览"
description: "C++23 继续完善现代 C++，引入了 std::expected、deducing this、std::print、std::mdspan 等重要特性"
tags: [C++23, 现代C++, expected, deducing this, print, mdspan, generator]
---
...
:::warning 编译器支持
C++23 的部分特性在当前主流编译器（GCC 14、Clang 18、MSVC 2022）中支持不完整。本章包含 **12 个可编译示例** 和 **7 个仅文档说明的特性**。
:::
...
| 40 | `40_expected.cpp` | std::expected | **核心** |
| 41 | `41_optional_monadic.cpp` | optional monadic | **核心** |
...
| 特性 | 说明 |
|------|------|
| Deducing this | 显式对象参数，消除 const/非const 重载 |
| std::print/println | 替代 cout 的现代输出 |
| std::mdspan | 多维数组视图 |
| std::flat_map/flat_set | 缓存友好的有序容器 |
| std::generator | 标准协程生成器 |
| static operator() | 无状态仿函数优化 |
| std::ranges 新增视图 | chunk、zip、enumerate 等 |
...
:::info 参考链接
[operator[]](https://en.cppreference.com/w/cpp/language/operator_member_access#Built-in_subscript_operator)
:::

### 03. size_t 字面量 (`03_size_t_literal.cpp`)

> **[→ 查看完整代码](./03-size-t-literal)**
...
:::tip 实用改进
`size_t` 字面量后缀终于消除了循环中常见的有符号/无符号比较警告。
:::
...
:::tip 核心特性
`std::expected` 是 C++23 最重要的标准库新增之一，提供了优雅的错误处理方式。
:::
...
> **[→ 查看完整代码](./41-optional-monadic)**

:::tip 函数式编程
Monadic 操作让 optional 的链式调用变得优雅，避免了嵌套的 if 检查。
:::
...
:::info 参考链接
[optional::and_then](https://en.cppreference.com/w/cpp/utility/optional/and_then) | [optional::transform](https://en.cppreference.com/w/cpp/utility/optional/transform) | [optional::or_else](https://en.cppreference.com/w/cpp/utility/optional/or_else)
:::

### 42. move_only_function (`42_move_only_function.cpp`)

> **[→ 查看完整代码](./42-move-only-function)**
...
> **[→ 查看完整代码](./44-unreachable)**

- 标记不可达的代码路径
- 允许编译器进行更积极的优化

:::warning 未定义行为
如果标记为 `std::unreachable()` 的代码实际被执行到，这是未定义行为。只在你 100% 确定代码不可达时才使用。
:::
...
:::warning 未定义行为
如果假设不成立是未定义行为。滥用 `[[assume]]` 可能导致难以调试的错误。
:::
...
```cpp title="传统方式 vs Deducing this"
// C++23 之前需要写两个版本：
struct Widget {
    int& get() { return data; }
...
:::info 参考链接
[deducing this](https://en.cppreference.com/w/cpp/language/member_functions#Explicit_object_parameter)
:::
...
:::info 参考链接
[std::print](https://en.cppreference.com/w/cpp/io/print) | [std::println](https://en.cppreference.com/w/cpp/io/println)
:::
...
:::info 参考链接
[std::flat_map](https://en.cppreference.com/w/cpp/container/flat_map) | [std::flat_set](https://en.cppreference.com/w/cpp/container/flat_set)
:::
...
:::info 参考链接
[static operator()](https://en.cppreference.com/w/cpp/language/operators#Function_call_operator)
:::
...
:::info 参考链接
[ranges](https://en.cppreference.com/w/cpp/ranges) | [views::chunk](https://en.cppreference.com/w/cpp/ranges/chunk_view) | [views::zip](https://en.cppreference.com/w/cpp/ranges/zip_view) | [views::enumerate](https://en.cppreference.com/w/cpp/ranges/enumerate_view)
:::
